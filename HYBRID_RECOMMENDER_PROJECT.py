...
##########################################################################################
# pip install surprise or conda install -c conda-forge scikit-surprise
import pandas as pd
...
# Adım 4: # index'te userID'lerin sutunlarda film isimlerinin ve değer olarakta ratinglerin bulunduğu dataframe için pivot table oluşturunuz.
...
user_movie_count.sort_values("movie_count", ascending=False).head(10)
...
final_df = pd.concat([movies_watched_df
                      [movies_watched_df.index.isin(users_same_movies)],
...
# Adım 2: movieId ve her bir filme ait tüm kullanıcıların weighted rating’lerinin ortalama değerini içeren recommendation_df adında yeni bir dataframe oluşturunuz.
...
recommend_movie = df[(df["userId"] == random_user) &
                     (df["rating"] == 5)].\
...
user_movie_df.loc[user_movie_df.index == random_user, user_movie_df.columns == recommend_movie]
user_movie_df.head(1)

filtered_movie_name = movie[movie["movieId"] == recommend_movie]["title"].values[0]
filtered_movie_name = user_movie_df[filtered_movie_name]


# Adım 4: Filtrelenen dataframe’i kullanarak seçili filmle diğer filmlerin korelasyonunu bulunuz ve sıralayınız.
movies_from_item_based = user_movie_df.corrwith(filtered_movie_name).sort_values(ascending=False)
